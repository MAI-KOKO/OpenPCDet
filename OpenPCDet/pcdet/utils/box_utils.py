...
def boxes3d_kitti_camera_to_lidar(boxes3d_camera, calib):
    ...
    xyz_lidar[:, 2] += h[:, 0] / 2 # kitti数据集的中心是底面中心，需要将其挪到实际3dbox的中心
...
def boxes_iou_normal(boxes_a, boxes_b):
    ...
    # 计算两个bev_box以及重叠区域的面积
    area_a = (boxes_a[:, 2] - boxes_a[:, 0]) * (boxes_a[:, 3] - boxes_a[:, 1])
    area_b = (boxes_b[:, 2] - boxes_b[:, 0]) * (boxes_b[:, 3] - boxes_b[:, 1])
    a_intersect_b = x_len * y_len
    # 计算iou
    iou = a_intersect_b / torch.clamp_min(area_a[:, None] + area_b[None, :] - a_intersect_b, min=1e-6) # (num_anchor, M_select)
    return iou
...
def boxes3d_lidar_to_aligned_bev_boxes(boxes3d):
    ...
    """
    """
        common_utils.limit_period(...)：这是一个函数调用，通常用于限制角度的范围，以确保角度值在指定的周期内(0.5~3.14) 

        * boxes3d[:, 6]：输入的角度数据
        * offset=0.5: 偏移量(弧度值)，用于调整周期的起始位置。在周期性角度处理中，通常希望指定一个起始位置，以确定周期的起点
        * period=np.pi: 指定的周期长度，通常是圆周率 π(约等3.14)
        * .abs()：这部分是对角度值进行取绝对值的操作，以确保所有的角度都在正数范围内
    """
    rot_angle = common_utils.limit_period(boxes3d[:, 6], offset=0.5, period=np.pi).abs()
    # torch.where(condition, x, y)：这个函数根据条件 condition，在 x 和 y 中选择元素
    # 具体操作是，对于每个位置，如果条件为真，就从 x 中选择元素，否则从 y 中选择元素
    # a = (rot_angle[:, None] < np.pi / 4) # a: (num_anchor, 1)
    choose_dims = torch.where(rot_angle[:, None] < np.pi / 4, boxes3d[:, [3, 4]], boxes3d[:, [4, 3]]) # (num_anchor, 2)
    # (x_center - l y_center - w)  (x_center + l y_center + w) 左上角坐标、右下角坐标
    aligned_bev_boxes = torch.cat((boxes3d[:, 0:2] - choose_dims / 2, boxes3d[:, 0:2] + choose_dims / 2), dim=1) # (num_anchor, 4)
...
def boxes3d_nearest_bev_iou(boxes_a, boxes_b):
    ...
    """
    boxes_bev_a = boxes3d_lidar_to_aligned_bev_boxes(boxes_a) # (num_anchor, 4) 3dBox投影到bev上面的坐标(xmin,ymin,xmax,ymax)
    boxes_bev_b = boxes3d_lidar_to_aligned_bev_boxes(boxes_b) # (M_select, 4)
