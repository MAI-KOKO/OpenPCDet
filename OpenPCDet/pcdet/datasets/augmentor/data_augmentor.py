...
class DataAugmentor(object):
        ...
        for cur_cfg in aug_config_list:
            if not isinstance(augmentor_configs, list):
                if cur_cfg.NAME in augmentor_configs.DISABLE_AUG_LIST:
                    continue
            # getattr(self, cur_cfg.NAME) 使用getattr函数，它接受两个参数
            # 第一个参数是要获取属性或方法的对象，这里是self
            # 第二个参数是属性或方法的名称，这里是cur_cfg.NAME。getattr函数将返回self对象的属性或方法cur_cfg.NAME的值
            # 此时cur_augmentor=random_world_flip
    ...
    def forward(self, data_dict):
        ...
        Returns:
        """
        # data_augmentor_queue在__init__定义
