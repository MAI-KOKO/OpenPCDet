...
class QueryAndGroup(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, xyz_batch_cnt: torch.Tensor,
        ...
        # idx: (M1 + M2 ..., nsample), empty_ball_mask: (M1 + M2 ...) 每个new_xyz周围采样nsample个点
        idx, empty_ball_mask = ball_query(self.radius, self.nsample, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt) # [4096, 16]
        grouped_xyz = grouping_operation(xyz, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, 3, nsample) [4096, 3, 16] 3：x, y, z
        grouped_xyz -= new_xyz.unsqueeze(-1)

        grouped_xyz[empty_ball_mask] = 0

        if features is not None:
            # pvrcnn: grouped_features: [B x 128 x 216, 128, 16]
            grouped_features = grouping_operation(features, xyz_batch_cnt, idx, new_xyz_batch_cnt)  # (M1 + M2, C, nsample) [4096, 1, 16] 1:反射率r
            grouped_features[empty_ball_mask] = 0
            if self.use_xyz:
                new_features = torch.cat([grouped_xyz, grouped_features], dim=1)  # (M1 + M2 ..., C + 3, nsample) [4096, 4, 16]
            else:
