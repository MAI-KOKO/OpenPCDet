...
def nms_gpu(boxes, scores, thresh, pre_maxsize=None, **kwargs):
    ...
    """
        scores.sort(0, descending=True) 执行了按第一个维度（通常是列维度）对 scores 进行降序排序，它返回了一个元组，其中包括排序后的值和排序后的索引
        [1] 取出了元组中的第二个元素，即排序后的索引
    """
    order = scores.sort(0, descending=True)[1] # order是scores的索引
    if pre_maxsize is not None:
        order = order[:pre_maxsize]

    boxes = boxes[order].contiguous() # 根据排序后的scores索引取出对应的box位置参数
    keep = torch.LongTensor(boxes.size(0)) # (N)
    num_out = iou3d_nms_cuda.nms_gpu(boxes, keep, thresh) # keep指针传递,返回的是满足nms条件(前num_out个)的boxes索引
    """
        Example: thresh = 60
            order ---- 4, 5, 7, 9, 6, 2 ...

            value_pre ---- 40, 89, 57, 76
            idx_pre ---- 0, 1, 2, 3
